...
export class BasisCash {
  ...
  constructor(cfg: Configuration) {
    ...
    this.BAC = new ERC20(deployments.Cash.address, provider, 'SUVC');
    this.BAS = new ERC20(deployments.Share.address, provider, 'SUVS');
    this.BAB = new ERC20(deployments.Bond.address, provider, 'SUVB');
  ...
  async getTokenPriceFromUniswap(tokenContract: ERC20): Promise<string> {
    ...
    const dai = new Token(chainId, DAI[0], DAI[1]);
  ...
  async stake(poolName: ContractName, amount: BigNumber): Promise<TransactionResponse> {
    ...
    const dd = await pool.stake(amount, {value})
    return dd //await pool.stake(amount, {value});
  }
